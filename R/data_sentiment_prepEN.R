...
tickers_port <- c("^spx", "^ndq")



price_data <- gpw$get_stooq_data(tickers_port)
colnames(price_data) <- c("Date", "SPX", "NDQ")
...
price_data |> saveRDS("data/prices_EN.RDS")



price_data <- readRDS("data/prices_EN.RDS")
...
my_sent_xts <- readRDS("data/my_sent_xtsEN.RDS")
...
my_data_xts |> saveRDS("data/sentimentEN.RDS")
