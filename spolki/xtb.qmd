---
title: "XTB S.A. (XTB)"
subtitle: "Profil spółki XTB — notowania, analiza, strategia"
categories: [Finanse, domy maklerskie i brokerzy]
pagetitle: "XTB S.A. (XTB) | Profil spółki GPW - Notowania i Analiza"
format:
  html:
    header-includes: |
      <meta name="description" content="XTB S.A. (XTB) - profil spółki na GPW. Aktualne notowania, wykres z SMA200, analiza działalności i strategii. Branża: domy maklerskie i brokerzy.">
      <meta name="keywords" content="XTB, akcje XTB, GPW, notowania, broker, Forex, CFD, xStation, fintech, inwestycje">
---

```{r setup, include=FALSE}
library(tidyverse)
library(plotly)
library(xts)
library(TTR)

get_stock_data <- function(ticker) {
  path <- "../data/prices100.RDS"
  if(!file.exists(path)) return(NULL)
  prices_all <- readRDS(path)
  col_idx <- which(names(prices_all) == ticker)
  if(length(col_idx) == 0) return(NULL)
  df <- prices_all[, c(1, col_idx)]
  names(df) <- c("date", "price")
  df <- df |> filter(!is.na(price))
  return(df)
}

stock_data <- get_stock_data("XTB")
last_row <- tail(stock_data, 1)
last_price <- last_row$price
last_date <- last_row$date
prev_price <- tail(stock_data$price, 2)[1]
change_pct <- round((last_price - prev_price) / prev_price * 100, 2)
y_ago_idx <- nrow(stock_data) - 252
if(y_ago_idx > 0) {
  y_ago_price <- stock_data$price[y_ago_idx]
  change_yy <- round((last_price - y_ago_price) / y_ago_price * 100, 2)
} else {
  change_yy <- "N/A"
}
color_dd <- if(change_pct >= 0) "success" else "danger"
color_yy <- if(is.numeric(change_yy) && change_yy >= 0) "success" else if(is.numeric(change_yy)) "danger" else "secondary"
val_yy <- if(is.numeric(change_yy)) paste0(change_yy, "%") else change_yy
```

::: {.company-header}
[← Spółki GPW](../spolki-gpw.html) | **Ticker:** XTB | **XTB S.A.** | **Siedziba:** Polska | **WWW:** [xtb.com](https://www.xtb.com){rel="nofollow"} | **Branża:** [Finanse](../spolki-gpw.html#category=Finanse) | **Podbranża:** [domy maklerskie i brokerzy](../spolki-gpw.html#category=domy maklerskie i brokerzy)
:::

```{r}
#| echo: false
#| results: asis
html_str <- paste0(
  '<div class="stock-metrics">',
  '<div class="stock-vbox stock-vbox-primary">',
  '<div class="stock-vbox-title">Cena (', format(last_date, '%Y-%m-%d'), ')</div>',
  '<div class="stock-vbox-value">', round(last_price, 2), '</div>',
  '</div>',
  '<div class="stock-vbox stock-vbox-', color_dd, '">',
  '<div class="stock-vbox-title">Zmiana d/d</div>',
  '<div class="stock-vbox-value">', change_pct, '%</div>',
  '</div>',
  '<div class="stock-vbox stock-vbox-', color_yy, '">',
  '<div class="stock-vbox-title">Zmiana y/y</div>',
  '<div class="stock-vbox-value">', val_yy, '</div>',
  '</div>',
  '</div>'
)
cat(html_str)
```

## Notowania

```{r}
#| echo: false
stock_xts <- xts(stock_data$price, order.by = as.Date(stock_data$date))
sma200 <- SMA(stock_xts, n = 200)
plot_ly(x = index(stock_xts), y = as.numeric(stock_xts), type = "scatter", mode = "lines", name = "Cena XTB") |>
  add_lines(x = index(stock_xts), y = as.numeric(sma200), name = "SMA 200", line = list(color = "orange")) |>
  layout(xaxis = list(title = "", rangeslider = list(visible = TRUE)), yaxis = list(title = "Cena (PLN)"), legend = list(orientation = "h", x = 0.1, y = 1.1), margin = list(t = 50))
```

## O spółce

XTB S.A. to jeden z największych na świecie brokerów instrumentów finansowych (Forex, CFD, akcje, ETF) notowanych na giełdzie. Polska spółka z sukcesem konkuruje na rynkach międzynarodowych, dostarczając autorską platformę transakcyjną xStation.

## Biznes

**Produkty i usługi:**

* **Forex:** Handel parami walutowymi z ponad 5700 instrumentami.
* **CFD:** Kontrakty na różnice kursowe na indeksy, akcje, surowce i kryptowaluty.
* **Akcje i ETF:** Dostęp do giełd polskich (GPW) i zagranicznych (NYSE, NASDAQ, LSE).
* **Plany inwestycyjne:** Automatyczne inwestowanie w ETFy.
* **Konta oszczędnościowe:** Oprocentowane depozyty dla klientów.

**Platforma xStation:**
Autorska platforma transakcyjna XTB (dostępna jako web, desktop i mobile) uznawana za jedną z najlepszych na rynku.

**Zasięg:**
Około 1 mln klientów w ponad 170 krajach, z silną pozycją w Europie, Ameryce Łacińskiej i Afryce.

## Strategia

Celem XTB jest zostać wiodącą aplikacją inwestycyjną w Europie (super-app finansowy). Strategia zakłada ciągłe poszerzanie oferty (np. o plany inwestycyjne, obligacje, konta oszczędnościowe) oraz inwestycje w marketing globalny z udziałem znanych ambasadorów marki.

:::: {.callout-important}
## Informacja prawna
Materiały publikowane na tej stronie mają charakter wyłącznie informacyjny i edukacyjny. Nie stanowią porady inwestycyjnej.
::::
